...
public class Model {
    ...
    public boolean maxTileExists() {
        ...
        for (int x = 0; x < boardSize; x++) {
            for (int y = 0; y < boardSize; y++) {
                if (tileHasMaxvalue(board.tile(x, y))) {
                    return true;
                }
            }
        }
    ...
    /* helper function to check if a tile has a max value */
    public boolean tileHasMaxvalue(Tile x) {
        return x != null && x.value() == MAX_PIECE;
    }
    ...
    public boolean atLeastOneMoveExists() {
        ...
        for (int x = 0; x < boardSize; x++) {
            for (int y = 0; y < boardSize; y++) {
                Tile tilePiece = board.tile(x, y);
                if (x > 0) {
                    Tile leftTile = board.tile(x - 1, y);
                    if (possibleMerge(tilePiece, leftTile)) {
                        return true;
                    }
                }
                if (y > 0) {
                    Tile bottomTile = board.tile(x, y - 1);
                    if (possibleMerge(tilePiece, bottomTile)) {
                        return true;
                    }
                }
            }
    ...
    /* helper function to check tile merge */
    public boolean possibleMerge(Tile x, Tile y) {
    ...
    public void moveTileUpAsFarAsPossible(int x, int y) {
        ...
        for (int i = y + 1; i < board.size() && !merge; i++) {
            Tile topTile = board.tile(x, i);
            if (topTile == null) {
                targetY = i;
            } else if (topTile.value() == myValue && !topTile.wasMerged()) {
                targetY = i;
                merge = true;
            }
        }
